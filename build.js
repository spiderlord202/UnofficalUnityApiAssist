    ...
    if (answers.action === "view"){
      ...
      logStream.write(answers.selectedItem[i]) 
			objselect(Object.keys(seedata${pathNum}).map(
				(item) => 
				{
          if (item === answers.selectedItem[i]) 
					{
            if (item === answers.selectedItem[i] && SelectedSafekeepNew.indexOf(selectedText(item)) != -1)
						  {
                //SelectedSafekeepNew.splice(SelectedSafekeepNew.indexOf(selectedText(item)), 1)
								return item;
              } else { 
                SelectedSafekeepNew.push(item)
                  return selectedText(item);
							}
            } else if (SelectedSafekeepNew.indexOf(selectedText(item)) != -1){
              return selectedText(item);
            } else {  
                return item; 
            }}));
